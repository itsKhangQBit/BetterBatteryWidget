...
Item {
    ...
    function getBlockerList() {
        ...
        sleepchk.runCMD("dbus-send --print-reply --dest=org.kde.Solid.PowerManagement /org/kde/Solid/PowerManagement/PolicyAgent org.kde.Solid.PowerManagement.PolicyAgent.ListInhibitions | sed -n '/string/p' | sed 's/string//g; s/\"//g; s/^[ \t]*//'");
        // how does it work?
        // we find "string" by sed and print it out (-n to be silent, we only want those lines)
        // then we replace "string" and '""' with nuthing :), and clear blank lines
    }
    ...
    function updateBlockerList(unparsedshit) { // lets just say that I parsed it myself..
        try {
            ...
            for (let i = 0; i < parsedshit.length; i+=2) { // jump 2 lines after we done
                ...
                let why = parsedshit[i + 1] || ""; // string "Playing video: "BFDI: TPOT 20: Alone"
                let name = rawName;
                //console.log(name)
            ...
            for (let appchoose = appListModel.count - 1; appchoose >= 0; appchoose--) {
                ...
                if (newApps.indexOf(currentApp) === -1) {
                    ...
                    // this is definitely more reliablee
                }
            }
